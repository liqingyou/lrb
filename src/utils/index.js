...
export function _showSelectDialog(sexList, cb) {
    ...
    const app = Vue.createApp({
        render() {
            return Vue.h(SelectDialog, { onCancel: remove, list: sexList, onOk: tempCb });
        }
    });
    const parent = document.createElement('div');
...
export function _showSimpleConfirmDialog(title, okCb, cancelCb, okText, cancelText) {
    ...
    const app = Vue.createApp({
        render() {
            return Vue.h(SimpleConfirmDialog, {
...
export function slideItemRender(props) {
    return function render(item, index, play, uniqueId) {
        ...
        switch (item.type) {
            case 'img':
                node = Vue.h('img', { src: item.src, style: 'height:100%;', alt: '' });
                break;
            case 'user':
                node = Vue.h(SlideUser, props);
                break;
            case 'send-video':
                node = Vue.h('video', { src: item.src, style: 'height:100%;' });
                break;
            default:
                node = Vue.h(BaseVideo, {
                    isPlay: play,
                    item: item,
                    index: index,
